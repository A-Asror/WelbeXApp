...
class BackendBaseSettings(pydantic.BaseSettings):
    TITLE: str = "FastAPI Template Application"
    VERSION: str = "0.1.0"
    TIMEZONE: str = "UTC"
    ...
    POSTGRES_URI: str = f'{DB_POSTGRES_SCHEMA}://{POSTGRES_USER}:{DB_POSTGRES_PASSWORD}@{DB_POSTGRES_HOST}' \
                        f':{DB_POSTGRES_PORT}/{DB_POSTGRES_NAME}'
    ...
    CELERY_BROKER_URL: str = decouple.config("CELERY_BROKER_URL", cast=str)
    CELERY_RESULT_BACKEND: str = decouple.config("CELERY_RESULT_BACKEND", cast=str)
